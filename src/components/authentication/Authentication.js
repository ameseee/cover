  ...
  render() {
    return (
      <div className="authentication-section">
        ...
        <h3>{this.state.isSigningUp ? 'Already have an account?' : 'Don\'t have an accout yet?'}</h3>
        <button onClick={this.switchToOtherForm}>{this.state.isSigningUp ? 'Sign In' : 'Sign Up'}</button>
      </div>
    );
  }
}
